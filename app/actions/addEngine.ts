...
const collectionId = "68071783002d63cc1dc2"; // Replace with your actual engine collection ID
const bucketId = "680747450034b2c09214";

export interface NewEngineProduct {
  title: string;
  description: string;
  features: string[];
  price: string;
...
export const addEngine = async (product: NewEngineProduct) => {
  try {
    let imageUrl = "";
    ...
    console.log("Engine product added:", response);
    return response;
  } catch (error) {
    console.error("Error adding engine product:", error);
    throw error;
  }
};
