...
import { getOilProducts } from "@/app/actions/getOil";

const OilChangePage = async () => {
  const oilProducts = await getOilProducts();
  console.log("oild products", oilProducts)

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6">Oil Change Products</h1>

      {oilProducts.length === 0 ? (
        <p>No oil products available.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {oilProducts.map((oil) => (
            <div key={oil.$id} className="border rounded-lg p-4 shadow hover:shadow-lg transition">
              {oil.imageUrl && (
                <img
                  src={oil.imageUrl}
                  alt={oil.title}
              ...
              <h2 className="text-xl font-semibold mb-2">{oil.title}</h2>
              <p className="text-gray-600 mb-2">{oil.description}</p>
              <p className="font-bold mb-2">${oil.price}</p>
              <p className="text-sm text-gray-500 mb-2">Supplier: {oil.supplier}</p>
              <div className="flex items-center text-yellow-500 mb-2">
                <span>⭐ {oil.rating} ({oil.reviews} reviews)</span>
              </div>
